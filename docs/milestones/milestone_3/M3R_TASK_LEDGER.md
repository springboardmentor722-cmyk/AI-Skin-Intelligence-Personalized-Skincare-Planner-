...
| M3R-P2-T4 | P2 | Budget optimization & alternatives | feat/m3r-p2-recommendation-engine | DONE | Hard-cap + alternative substitution integrated into GET `/recommendations/me` response. Budget-flagged products (over_budget: true) paired with cheaper alternatives via alternative_for_product_id + alternative_category. Inline, no separate endpoint call needed. |
