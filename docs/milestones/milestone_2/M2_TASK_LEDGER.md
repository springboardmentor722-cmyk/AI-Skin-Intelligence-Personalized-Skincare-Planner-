...
| M2-P12-T01 | P12 | Suitability/interaction/allergy/education (extend, not rebuild) | feat/m2-ingredient-intelligence | DONE | MILESTONE 2.docx §5 | Real gap found (ADR-030): structured allergy list (P7) was never wired into suitability, no synonym/INCI matching existed, interaction matrix was never consulted by routine generation — all three closed |
| M2-P12-T02 | P12 | Ingredient Analyzer + Ingredient Database frontend | feat/m2-ingredient-intelligence | DONE | UI_SPEC nav trees | `user`-role Ingredient Analyzer already existed; built the shared consultant/dermatologist/admin Ingredient Database (was a P2 stub in all three), `nav-config.ts` `built: true` |
