...
import { getConsultantUserDetail, getProducts, saveProductRecommendation, getUserProfile } from "../api/dashboard";
...
export default function ConsultantProductRecommendPage() {
  ...
  const { data: userDetail, isLoading: loadingClient } = useQuery({
    queryKey: ["userDetail", clientId],
    queryFn: () => getConsultantUserDetail(clientId),
  });
  ...
  const mutation = useMutation({
    mutationFn: saveProductRecommendation,
    onSuccess: () => {
      queryClient.invalidateQueries(["userDetail", clientId]);
      queryClient.invalidateQueries(["products"]);
      navigate("/consultant/products");
    }
  });
  ...
  // Dynamic Concerns Formatting (removes underscores & capitalizes cleanly)
  ...
  const handleAddProduct = (prod) => {
    const newItem = {
      ...
      purpose: "Targeted hydration & barrier care",
      quantity: "Pea size",
      time: "AM"
    };
  ...
  const handleSave = () => {
    const allProducts = [
      ...morningRoutine.map((p, i) => ({ ...p, routine: "morning", step_number: i + 1 })),
      ...nightRoutine.map((p, i) => ({ ...p, routine: "night", step_number: i + 1 })),
      ...afternoonRoutine.map((p, i) => ({ ...p, routine: "afternoon", step_number: i + 1 })),
      ...weeklyTreatments.map((p, i) => ({ ...p, routine: "weekly", step_number: i + 1 }))
    ];
        ...
        <Box sx={{ maxWidth: 1720, margin: "0 auto" }}>
          <Stack direction={{ xs: "column", md: "row" }} alignItems={{ xs: "flex-start", md: "center" }} justifyContent="space-between" spacing={2}>
            <Stack direction="row" spacing={2} alignItems="center">
              <IconButton onClick={() => navigate("/consultant/products")} sx={{ border: `1px solid ${cBorderAccent}`, width: 40, height: 40, borderRadius: "10px" }}>
                <ArrowBack sx={{ color: cText, fontSize: 18 }} />
              </IconButton>
              <Box>
                <Stack direction="row" alignItems="center" spacing={1.5} flexWrap="wrap">
                  <Typography sx={{ fontSize: 24, fontWeight: 800, color: cText, letterSpacing: "-0.5px" }}>Product Recommendation</Typography>
                  <Chip label={info?.full_name || "K harichandana"} size="small" sx={{ backgroundColor: "#F3E8FF", color: cPrimary, fontWeight: 800, borderRadius: "6px" }} />
                ...
                <Typography sx={{ fontSize: 13, color: cSecondaryText, mt: 0.3 }}>Recommend personalized skincare products based on AI assessment and consultant expertise.</Typography>
              </Box>
            </Stack>
          ...
          <Stack spacing="32px" width="100%">
            ...
            <Box sx={{ display: "flex", flexDirection: { xs: "column", lg: "row" }, gap: "28px", alignItems: "flex-start", width: "100%" }}>
              ...
              <Box sx={{ width: { xs: "100%", lg: "340px" }, flexShrink: 0, display: "flex", flexDirection: "column", gap: "24px" }}>
                ...
                <Paper sx={cardStyle}>
                  ...
                  <Stack direction="row" spacing={2} alignItems="center" mb={3}>
                    <Avatar src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150&auto=format&fit=crop&q=80" sx={{ width: 64, height: 64, border: `2px solid ${cPrimary}30` }}>
                      {initials(info?.full_name || "Client")}
                    </Avatar>
                    <Box>
                      <Typography sx={{ fontSize: 18, fontWeight: 800, color: cText }}>{info?.full_name || "Client"}</Typography>
                      <Typography sx={{ fontSize: 13, color: cSecondaryText, mt: 0.2 }}>{info?.age ? `${info.age} yrs` : "20 yrs"} • {info?.gender || "Female"}</Typography>
                      <Chip label={`ID: ${clientId?.substring(0,8).toUpperCase() || "CLIENT"}`} size="small" sx={{ mt: 0.5, borderRadius: "6px", backgroundColor: "#F3E8FF", color: cPrimary, fontWeight: 800, fontSize: 10 }} />
                    </Box>
                  </Stack>
              ...
              <Box sx={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: "24px" }}>
                ...
                {/* Recommended Product Gallery Cards (4-COLUMN RESPONSIVE GRID) */}
                <Paper sx={cardStyle}>
                  ...
                  {/* 4-COLUMN SLEEK RESPONSIVE GRID */}
                  <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr", md: "repeat(4, 1fr)" }, gap: "20px" }}>
                    {recommendedProducts.map((prod, i) => (
                      <Paper key={`gal-${prod.id}`} sx={{ p: 2, borderRadius: "16px", border: `1px solid ${cBorder}`, position: "relative", display: "flex", flexDirection: "column", justifyContent: "space-between", backgroundColor: "#FFF", transition: "all 0.25s", '&:hover': { borderColor: cPrimary, transform: "translateY(-4px)", boxShadow: "0 10px 25px rgba(139, 92, 246, 0.12)" } }}>
                        <Chip label={`${96 - i * 2}% Match`} size="small" sx={{ position: "absolute", top: 12, right: 12, backgroundColor: "#D1FAE5", color: "#059669", fontWeight: 800, fontSize: 10, height: 20, zIndex: 1 }} />
                        
                        <Box sx={{ width: "100%", height: 120, borderRadius: "12px", backgroundColor: "#FAFAFB", display: "flex", alignItems: "center", justifyContent: "center", mb: 1.5, p: 1, border: `1px solid ${cBorder}` }}>
                        ...
                        <Box flexGrow={1} display="flex" flexDirection="column" justifyContent="space-between">
                          <Box mb={1}>
                            <Typography sx={{ fontSize: 11, fontWeight: 800, color: cPrimary, textTransform: "uppercase", letterSpacing: 0.5, mb: 0.3 }}>{prod.brand}</Typography>
                            <Typography sx={{ fontSize: 13, fontWeight: 800, color: cText, lineHeight: 1.3, height: 36, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                              {prod.name}
                            </Typography>
                          </Box>
                          
                          <Stack direction="row" justifyContent="space-between" alignItems="center" mt={1} mb={1.5}>
                            <Typography sx={{ fontSize: 14, fontWeight: 800, color: cText }}>₹{350 + i * 200}</Typography>
                            <Chip label={prod.category || "Serum"} size="small" sx={{ fontSize: 10, fontWeight: 700, backgroundColor: "#F3E8FF", color: cPrimary, borderRadius: "6px" }} />
                          </Stack>

                          <Button fullWidth size="small" onClick={() => handleAddProduct(prod)} variant="outlined" sx={{ textTransform: "none", fontWeight: 700, fontSize: 11, borderRadius: "8px", borderColor: cBorderAccent, color: cPrimary, '&:hover': { backgroundColor: `${cPrimary}0A` } }}>
                            View Details
                          </Button>
                        </Box>
                      </Paper>
      ...
      <Box sx={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 20, backgroundColor: "rgba(255, 255, 255, 0.95)", backdropFilter: "blur(20px)", borderTop: `1px solid ${cBorderAccent}`, boxShadow: "0 -10px 30px rgba(139, 92, 246, 0.1)", px: { xs: 3, md: 5 }, py: 2 }}>
        <Box sx={{ maxWidth: 1720, margin: "0 auto" }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            <Button sx={{ color: cSecondaryText, fontWeight: 700, textTransform: "none", fontSize: 14, px: 3, borderRadius: "10px", border: `1px solid ${cBorder}` }} onClick={() => navigate("/consultant/products")}>
              Cancel
            </Button>
            <Stack direction="row" spacing={2}>
