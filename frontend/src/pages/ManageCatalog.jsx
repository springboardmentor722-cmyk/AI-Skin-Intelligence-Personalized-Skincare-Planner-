...
export default function ManageCatalog({ type }) {
  ...
  const load = async () => { setLoading(true); setError(""); try { const { data } = await api.get(`/${type}/admin`, { params: { search } }); setItems(data); } catch (e) { setError(e.response?.data?.detail || `Unable to load ${label.toLowerCase()}.`); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
