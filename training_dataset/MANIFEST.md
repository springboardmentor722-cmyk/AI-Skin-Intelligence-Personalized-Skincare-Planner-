...
| 4 | Sephora Products (image backfill only) | `yamqwe/sephora-products` | `training_dataset/raw/sephora-images/` | `sephora.csv` | `backend/app/services/admin/ingest/enrich_product_images.py` → downloads + re-hosts through `app/core/storage.py`, backfills `products.image_url` (an S3 key, not the source URL) only, exact-match by normalized (brand, product_name) against every product still missing an image | **Ingested 2026-08-02, re-run 2026-08-04 (ADR-044)** — 39/2,409 matched originally; re-run against the grown 6,699-row catalog (34 products still held a real `image_url` at re-run time) matched 256 more. Small overlap by design: only the same SKU appearing in both scrapes matches; no fuzzy matching, to avoid ever showing a different real product's photo. Live hotlinking to Sephora's CDN was considered and rejected as not production-safe. |
| 5 | Skincare Products Clean Dataset | `eward96/skincare-products-clean-dataset` | `training_dataset/raw/skincare-clean/` | `skincare_products_clean.csv` | `backend/app/services/admin/ingest/ingest_skincare_clean.py` → `products`/`ingredients`/`product_ingredients` (Postgres) | **Ingested 2026-08-03** — 1,138 lookfantastic.com-scraped skincare products loaded (`make ingest-skincare-clean`). No brand_name column in source — brand extracted from product_name via longest-match-first against 44 known multi-word brands (La Roche-Posay, Elizabeth Arden, First Aid Beauty, etc.), with fallback to "The X" pattern, then single leading word. GBP pricing (£ symbol, requires latin-1 encoding). Zero nulls in any column. |
...
| 8 | Open Beauty Facts | `openfoodfacts/openbeautyfacts` | `training_dataset/raw/open-beauty-facts/` | `en.openbeautyfacts.org.products.tsv` | Landing only for full product ingest. `backend/app/services/admin/ingest/enrich_product_images.py` (ADR-044) reads it as a third image-only source, exact-match by normalized (brand, product_name) against products still missing an image. | **Downloaded 2026-08-03**, no price field anywhere in the dataset (fails the mandatory-field gate every other dataset here honors), dominated by non-skincare categories — still not a product-ingest source. 1,352/4,304 rows carry a real `image_url`; wired in as an image-only source 2026-08-04 (ADR-044) — 0 matched against this catalog on the first run (different brand-naming conventions, e.g. comma-joined co-brands; a real result of exact-match discipline, not a bug). |
| 9 | Dermstore Skincare Products & Ingredients | `crawlfeeds/dermstore-skincare-products-and-ingredients-dataset` | `training_dataset/raw/dermstore/` | `dermstore_data.json` | Landing only for full product ingest. `backend/app/services/admin/ingest/enrich_product_images.py` (ADR-044) reads it as a third image-only source, same exact-match discipline. | **Downloaded 2026-08-03**, 126 rows, not skincare-exclusive, no reliable category signal (per-product breadcrumb only) — still not a product-ingest source. All 126 rows carry a real `images` field; wired in as an image-only source 2026-08-04 (ADR-044) — 1 matched. |
...
4. Dataset #4: `make enrich-product-images` (downloads #4, then also reads #8/#9
   already on disk, matches all three in turn, backfills `image_url` only — safe to
   re-run, only ever touches rows where it's still NULL). Run #1 first; #4 has
   nothing to match against otherwise. #8/#9 must already be present under their
   folders below (already downloaded as of 2026-08-03) — this pipeline doesn't
   fetch them itself.
