...
async def _read_with_steps(db: AsyncSession, routine: Routine) -> RoutineRead:
    ...
    *per step* (a RoutineProduct query, then a get_products_by_ids call) instead of
    ...
    # ready-to-use URL, so this can't be skipped).
    product_reads_by_id = {
        product_id: await recommendations_service.resolve_product_read(product)
...
async def search_products_for_edit(
    ...
    return await recommendations_service.resolve_product_reads(matches[:10])
