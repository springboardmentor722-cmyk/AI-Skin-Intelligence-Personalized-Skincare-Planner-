...
//
// M3R Phase 5 Task 5: extracted from the user's own
// web/app/(user)/routine/edit/[routineId]/page.tsx so the professional edit
// screen (web/components/clinical-review/professional-routine-edit-view.tsx)
// reuses the exact same editing logic instead of duplicating it, parametrized by
// `clientUserId` to pick which endpoint family to call. NOTE: Task 4's
// assignment-gated backend only added product-search/add/update/delete for a
// client's routine — no `/api/v1/clients/{user_id}/routines/{routine_id}/steps/reorder`
// endpoint exists, so reordering is only offered when editing your own routine
// (`clientUserId` is undefined); a professional's edit view hides the move
// controls rather than silently failing to persist a reorder.
...
function ProductPicker({
  ...
  const searchQuery = useQuery({
    ...
    queryFn: async () => {
      const { data } = clientUserId
        ? await api.GET("/api/v1/clients/{user_id}/routines/products/search", {
            params: { path: { user_id: clientUserId }, query: { category, q: debouncedQuery } },
          })
        : await api.GET("/api/v1/routines/products/search", {
...
export interface RoutineEditorProps {
  /** The routine being edited — caller (page/view) owns fetching + loading/not-found states. */
  routine: RoutineRead;
  /** Where the back arrow links and where a successful save redirects to. */
  backHref: string;
  /**
...
export function RoutineEditor({ routine, backHref, clientUserId }: RoutineEditorProps) {
  ...
  const [syncedRoutineId, setSyncedRoutineId] = useState<number>(routineId);
  if (routine.routine_id !== syncedRoutineId) {
    ...
    try {
      for (const stepId of deletedStepIds) {
        const { error } = clientUserId
          ? await api.DELETE("/api/v1/clients/{user_id}/routines/steps/{step_id}", {
              params: { path: { user_id: clientUserId, step_id: stepId } },
            })
          : await api.DELETE("/api/v1/routines/steps/{step_id}", {
              params: { path: { step_id: stepId } },
            });
        if (error) throw new Error("Couldn't remove one of the steps.");
      }
      ...
      for (const step of existingSteps) {
        ...
        if (
          ...
          const { error } = clientUserId
            ? await api.PATCH("/api/v1/clients/{user_id}/routines/steps/{step_id}", {
                params: { path: { user_id: clientUserId, step_id: step.stepId! } },
                body: {
            ...
            : await api.PATCH("/api/v1/routines/steps/{step_id}", {
      ...
      for (const step of steps.filter((s) => s.isNew)) {
        const { error } = clientUserId
          ? await api.POST("/api/v1/clients/{user_id}/routines/{routine_id}/steps", {
              params: { path: { user_id: clientUserId, routine_id: routineId } },
              body: { step_name: step.stepName, product_id: step.productId },
            })
          : await api.POST("/api/v1/routines/{routine_id}/steps", {
              params: { path: { routine_id: routineId } },
              body: { step_name: step.stepName, product_id: step.productId },
            });
      ...
      await queryClient.invalidateQueries({
        queryKey: clientUserId ? ["clinical-review", "client", clientUserId] : ["routines", "me"],
      });
