...
// web/designs/wireframes/app-ingredients.html — real content only. The wireframe's
// per-card "FOR YOU: SUITABLE" badge needs a per-ingredient suitability call
// (GET /ingredients/{id}/suitability/me is single-ingredient, no batch endpoint
// exists per milestone_3.md's API table) — fetching that for every visible card
// would be an N+1 query pattern, so suitability lives on the detail page instead,
// where it belongs to exactly one ingredient. Per-card descriptive prose and the
// 3D molecule renders are also dropped — no `description`/image column exists on
// `ingredients` (database_schemas/..._postgresql_schema_v3.sql).
const CATEGORIES = [
  "Retinoids",
  "Niacinamide",
  "Vitamin C",
...
export default function IngredientsPage() {
  ...
  // Base UI's Select warns if it switches from uncontrolled (undefined) to
  // controlled (a string) after mount — "" (never undefined) keeps it controlled
  // from the first render, with "" standing for "nothing picked yet".
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-on-surface-variant mt-1 max-w-2xl font-sans text-sm">
            Explore ingredient science and how each one fits your skin profile.
          </p>
        </div>
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-on-surface-variant font-sans text-xs">
              ...
              dermatologist or consultant for anything applied to compromised or
              reactive skin.
            </p>
          </div>
        </DialogContent>
