...
## Corpus Check
- 521 files · ~1,073,749 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4011 nodes · 9255 edges · 243 communities (213 shown, 30 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 231 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d3ae7667`
...
1. `cn()` - 241 edges
2. `get_db()` - 81 edges
3. `create_profile()` - 79 edges
4. `require_role()` - 78 edges
5. `SkinProfileCreate` - 76 edges
6. `get_mongo_db()` - 71 edges
7. `require_user()` - 66 edges
8. `Button()` - 59 edges
9. `get_or_generate_routines()` - 48 edges
...
## Communities (243 total, 30 thin omitted)

### Community 0 - "get_db"
Cohesion: 0.07
Nodes (83): require_role(), get_db(), AsyncSession, approve_verification(), assign_consultant_client(), create_audit_log(), deactivate_professional(), get_audit_logs() (+75 more)
...
Cohesion: 0.07
Nodes (61): get_interaction(), Interaction, TypedDict, Curated pairwise ingredient interaction rules (M3-B, PDF Module 5 "interaction, None means "no curated interaction documented" — never a fabricated verdict, get_ingredient(), get_interactions(), get_my_suitability() (+53 more)

### Community 3 - "button.tsx"
Cohesion: 0.08
Nodes (32): LatencyStatsRead, ASSIGNABLE_ROLES, BetterAuthUser, ListUsersResponse, Role, QueueItem, ROLE_OPTIONS, STATUS_OPTIONS (+24 more)
...
Cohesion: 0.08
Nodes (64): AuditLog, Polymorphic across ConsultantProfile/DermatologistProfile via `owner_user_id`,, General-purpose system audit log — Admin's "Audit Logs"/"Activity Logs"     scr, VerificationDocument, apply_verification_action(), assign_client(), create_document(), delete_document() (+56 more)

### Community 7 - "cn"
Cohesion: 0.09
Nodes (39): get_admin_analytics(), get_my_analytics(), Any, AsyncSession, Depends, ge, le, Query (+31 more)

### Community 8 - "Base"
Cohesion: 0.08
Nodes (61): _IngredientSeed, main(), _ProductSeed, TypedDict, Idempotent local/dev seed data — `make seed` / `python -m app.db.seed`.  Seeds, seed_ingredients(), seed_products(), Ingredient (+53 more)
...
Cohesion: 0.08
Nodes (43): AsyncIOMotorClient, AsyncIOMotorDatabase, ingest_for_concern(), main(), Real PubMed ingestion — `make ingest-knowledge` / `python -m app.db.ingest_knowl, get_mongo_client(), get_mongo_db(), Any (+35 more)

### Community 12 - "test_dermatologist_profile_service.py"
Cohesion: 0.04
Nodes (94): ADR-0016, AdminDashboardPage(), AuditLogEntry, DashboardStatsResponse, formatAction(), PlatformCounts, TopConcernStat, ADR-0023 (+86 more)

### Community 13 - "sidebar.tsx"
Cohesion: 0.10
Nodes (23): AssessmentConcernPriority, AssessmentContext, AssessmentContextValue, AssessmentProvider(), AssessmentSnapshot, AssessmentState, commit(), DEFAULT_STATE (+15 more)

### Community 14 - "test_routines_service.py"
Cohesion: 0.13
Nodes (56): _current_season(), get_or_generate_routines(), ProductRead, Deterministic, `hash(user_id)`-seeded routine generation (ADR-007 spirit) — no, search_products_for_edit(), SkinProfileConcernInput, SkinProfileCreate, create_profile() (+48 more)
...
Cohesion: 0.12
Nodes (27): _hydration_score(), _lifestyle_score(), Any, H — Hydration (10%): "evaluated against a 3.0 L daily fluid benchmark"     (`HY, A — Adherence (20%): "computed from the active 14-day completion logs;     defa, L — Lifestyle (20%): "evaluated against daily stress level and sun exposure, _routine_adherence_score(), Milestone 1 audit: scores/service.py — the module computing the weighted Skin H (+19 more)
...
Cohesion: 0.06
Nodes (47): CATEGORIES, EditableStep, EditRoutinePage(), ProductPicker(), ProductRead, RoutineRead, toEditableSteps(), useDebouncedValue() (+39 more)

### Community 19 - "signup/page.tsx"
Cohesion: 0.04
Nodes (76): InsightsPage(), RosterColumn, RosterTable(), AllergyIngredient, AllergyIngredientSelect(), ADR-0026, Breadcrumb(), BreadcrumbEllipsis() (+68 more)
...
Cohesion: 0.08
Nodes (50): Taxonomy/reference table — seeded, rarely written after (Normal/Dry/Oily/     C, One row per saved profile version — `is_current` marks the active one, prior, Milestone 2 P7 — the allergy list as structured ingredient ids (P12's allergy, SkinProfile, SkinProfileAllergy, SkinType, AllergyIngredientRead, LifestyleLogCreate (+42 more)

### Community 22 - "postgres.py"
Cohesion: 0.09
Nodes (56): get_my_progress_logs(), get_my_progress_photos(), get_my_progress_summary(), Any, AsyncSession, Depends, File, ge (+48 more)
...
Cohesion: 0.09
Nodes (50): AssessmentSubmission, Milestone 2 P9 — the immutable raw snapshot POST /api/v1/assessment/submit, Any, AsyncSession, Depends, submit_assessment(), AssessmentConcernInput, AssessmentLifestyleInput (+42 more)
...
Cohesion: 0.08
Nodes (27): ADR-0005, CORE_TABS, MyRoutinePage(), RoutineRead, RoutineStepCard(), RoutineStepRead, RoutineType, RoutineChecklistCard() (+19 more)

### Community 31 - "routines/service.py"
Cohesion: 0.06
Nodes (76): Deterministic RNG for M1 AI stubs (ADR-007: 'deterministic, hash(user_id)-seeded, seeded_random(), PipelineStep, NamedTuple, Milestone 2 P11 (MILESTONE 2.docx §"Dynamic Routine Generator" / §4 "Personalize, apply_safety_guardrails(), assert_sunscreen_present(), GeneratedStep (+68 more)

### Community 32 - "storage.py"
Cohesion: 0.13
Nodes (30): build_key(), _client_kwargs(), FileValidationError, get_presigned_url(), Exception, S3-compatible object storage adapter (docs/ARCHITECTURE.md §7, database_schemas/, `{prefix}/user_{id}/{uuid}_{filename}` — matches the infra doc's     `{entity_t, `allowed_content_types` is the caller's own allowlist for its use case (e.g. (+22 more)

### Community 33 - "test_consultant_profile_service.py"
Cohesion: 0.07
Nodes (36): ADR-0025, AssessmentBasicsPage(), GOALS, AssessmentConcernsPage(), AssessmentLifestylePage(), firstOf(), SUN_EXPOSURE_OPTIONS, ANALYSIS_POINTS (+28 more)
...
Cohesion: 0.12
Nodes (44): delete(), delete_my_document(), get_my_profile(), list_my_documents(), Any, AsyncSession, Depends, DocumentType (+36 more)
...
Cohesion: 0.12
Nodes (20): CheckInPage(), firstOf(), HydrationRestForm(), HydrationRestFormProps, LifestyleLogRead, todayIso(), SkinScoreTrendChartProps, TrendChart() (+12 more)

### Community 42 - "app/page.tsx"
Cohesion: 0.11
Nodes (14): FEATURES, FeaturesGrid(), FinalCtaSection(), HowItWorksSection(), STEPS, FOOTER_COLUMNS, LandingFooter(), PricingSection() (+6 more)

### Community 43 - "consultant_profile/router.py"
Cohesion: 0.13
Nodes (17): Base, Shared declarative base. Each service owns its own tables (ADR-005) — a service, do_run_migrations(), include_object(), Any, In this scenario we need to create an Engine     and associate a connection wit, Run migrations in 'online' mode., Run migrations in 'offline' mode.      This configures the context with just a (+9 more)

### Community 44 - "assessment/context.tsx"
Cohesion: 0.08
Nodes (26): IngredientDetailPage(), ProductsComparePage(), ProductDetailPage(), ProductsPage(), firstOf(), RecommendationRead, RecommendationsPage(), SORT_ITEMS (+18 more)
...
Cohesion: 0.15
Nodes (28): Gates *operational* consultant/dermatologist endpoints (M2+) on the matching, require_user(), require_verified_professional(), Any, AsyncClient, AsyncSession, RBAC — `require_role`/`require_verified_professional` unit coverage plus a repr, test_admin_only_routes_reject_non_admin_roles() (+20 more)
...
Cohesion: 0.11
Nodes (20): LoginForm(), safeRedirectTarget(), ROLE_CARDS, SignupPage(), STRENGTH_COLORS, AuthSplitLayout(), GoogleIcon(), Label() (+12 more)
...
Cohesion: 0.06
Nodes (30): ADR-001 — Drop the runtime graph database, ADR-002 — Better Auth is the single auth authority, ADR-003 — User IDs are strings (TEXT); identity tables are Better-Auth-owned, ADR-004 — Frontend: Next.js + Tailwind + shadcn/ui, Recharts, Stitch designs, ADR-005 — Modular monolith for M1–M3, containers at M4, ADR-006 — Adopt Graphify (the dev tool) for agent context persistence, ADR-007 — AI is stubbed until Milestone 2, ADR-008 — "Frosted Lab Glass" design language (glassmorphism on navy/blue/teal) (+22 more)
...
Cohesion: 0.15
Nodes (21): NAV_LINKS, ThemeToggle(), Avatar(), AvatarBadge(), AvatarFallback(), AvatarGroup(), AvatarGroupCount(), AvatarImage() (+13 more)
...
Cohesion: 0.17
Nodes (21): SkinScore, compute_and_store_score(), count_all_assessments(), get_recent_scores(), get_score_by_id(), AsyncSession, ScoreRead, GET /api/v1/assessment/score/{id} (P10) — ownership-checked: a `score_id`     b (+13 more)
...
Cohesion: 0.12
Nodes (15): ADR-0022, EXTRA_TITLES, FooterConfig, NAV_ITEMS, NavItem, NavSection, RAW_NAV, RawNavItem (+7 more)
...
### Community 98 - "sonner"
Cohesion: 0.08
Nodes (24): get_settings(), Env vars documented in /.env.example — read from there, not invented here., Settings, error_envelope(), Request, _identity(), Receive, Request (+16 more)
...
Cohesion: 0.04
Nodes (67): Action, ACTION_LABELS, FIELD_LABELS, formatValue(), REASON_REQUIRED, Role, VerificationReviewPage(), ConsultantBackgroundPage() (+59 more)
...
Cohesion: 0.16
Nodes (23): get_latency_stats(), LatencyStats, _percentile(), NamedTuple, Best-effort — a Redis outage degrades this rolling metric, it must never     ta, Degrades to an honest empty reading (never a guessed number) if Redis is     un, record_latency(), get_redis() (+15 more)
...
Cohesion: 0.15
Nodes (13): Milestone 2 P10 (MILESTONE 2.docx §"2. Weighted Skin Health Scoring Engine") —, derive_skin_age(), None for an unset or unrecognized band — Skin Age isn't derivable without a, Skin Age (decision C6, ADR-028) — derived from the Condition sub-score and, representative_age_for_group(), score_band(), _skin_age_and_band(), test_derive_skin_age_ages_up_by_the_max_penalty_for_a_zero_condition_score() (+5 more)
...
Cohesion: 0.23
Nodes (10): GlassTopbar(), NavUser(), HeroSection(), LandingNavbar(), Coords, useBrowserCoords(), useWeatherUV(), getSettingsHref() (+2 more)

### Community 187 - ".__call__"
Cohesion: 0.35
Nodes (6): ScoringWeights, BaseModel, ScoreRead, ScoreWeightsRead, get_active_weights(), test_get_active_weights_matches_scoring_weights_sum_to_one()
...
Cohesion: 0.33
Nodes (9): count_completed_steps_by_user(), _day_start(), get_completed_step_ids(), list_recent_routine_logs(), Any, date, Interface function (ADR-005) — bulk, cross-user sibling of     `list_recent_rout, toggle_step_completion() (+1 more)
...
## Knowledge Gaps
- **1056 isolated node(s):** `WorkerSettings`, `skinlytics-backend`, `verification`, `jwks`, `outbox` (+1051 more)
...
- **Why does `cn()` connect `signup/page.tsx` to `test_consultant_profile_service.py`, `button.tsx`, `tailwind-merge`, `chart.tsx`, `appearance-settings.tsx`, `assessment/context.tsx`, `test_dermatologist_profile_service.py`, `utils.ts`, `get_my_score`, `[routineId]/page.tsx`, `product-recommendation-card.tsx`, `routine/page.tsx`, `consultant-onboarding.ts`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `dependencies` connect `dependencies` to `role-sidebar-labels.spec.ts`, `consultant/treatment-protocols/page.tsx`, `dermatologist/assessments/page.tsx`, `admin/products/page.tsx`, `consultant/assessments/page.tsx`, `zod`, `recharts`, `get_my_score`, `conftest.py`, `tailwind-merge`, `tw-animate-css`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
- **Why does `get_db()` connect `get_db` to `sonner`, `ingredients/service.py`, `(user)/dashboard/page.tsx`, `test_clinical_review_service.py`, `dermatologist-onboarding/onboarding-shell.tsx`, `cn`, `consultant_profile/router.py`, `consultant-onboarding/context.tsx`, `postgres.py`, `get_mongo_db`, `.__call__`, `nav-config.ts`, `routines/service.py`?**
  _High betweenness centrality (0.016) - this node is a cross-community bridge._
